...
export default function GradeOneFee (props) {
  let {closeProfile} = props
  const complete = props.idsrCases.filter(idsrCase => !!idsrCase.complete && idsrCase.feeInfo.class === 'Grade One')
    ...
    return docs.length === 0
      ...
      : (
        <table className='profile'>
          <tbody>
            {docs.map((doc) => <SemesterDataRow key={doc._id} doc={doc} {...this.props} />)}
            {docs.map((doc) => <PeriodDataRow key={doc._id} doc={doc} {...this.props} />)}
            {docs.map((doc) => <InstallmentDataRow key={doc._id} doc={doc} {...this.props} />)}
            {docs.map((doc) => <AmountDataRow key={doc._id} doc={doc} {...this.props} />)}
          </tbody>
          {pagination}
        </table>
    )
...
function IdDataRow (props) {
  ...
  return (
    ...
    <td>{doc.registrationInfo.studentID}</td>
    </tr>
  )
}
...
function SemesterDataRow (props) {
  let {doc} = props

  return (
    <tr className='separate'>
    <td className='title-padding'>Semester:</td>
    <td>{doc.feeInfo.semester}</td>
    </tr>
  )
}
function PeriodDataRow (props) {
  let {doc} = props

  return (
    <tr className='separate'>
    <td className='title-padding'>Period:</td>
    <td>{doc.feeInfo.period}</td>
    </tr>
  )
}
function InstallmentDataRow (props) {
  let {doc} = props

  return (
    <tr className='separate'>
    <td className='title-padding'>Installment:</td>
    <td>{doc.feeInfo.installment}</td>
    </tr>
  )
}
function AmountDataRow (props) {
  let {doc} = props

  return (
    <tr className='separate'>
    <td className='title-padding'>Amount:</td>
